...
public class BonflowerHoneyItem extends Item {
	...
	public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
		Text tooltipText = Text.translatable("item.floristry.bonflower_honey.tooltip").formatted(Formatting.GRAY).formatted(Formatting.ITALIC);
		tooltip.add(tooltipText);
	}

	@Override
	public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
		StatusEffectInstance effect = new StatusEffectInstance(StatusEffects.NIGHT_VISION, 3600); // 3 minutes
